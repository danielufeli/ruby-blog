...
RSpec.describe 'Posts API', type: :request do
  ...
  describe 'POST /posts' do
    # valid payload
    let(:valid_attributes) { { title: 'Daniel Ufeli', content: 'Lorem', created_by: '1' } }

    context 'when the request is valid' do
      before { post '/posts', params: valid_attributes }

      it 'creates a post' do
        expect(json['title']).to eq('Daniel Ufeli')
      end
    ...
    context 'when the request is invalid' do
      ...
      it 'returns a validation failure message' do
        expect(response.body)
          .to match(/Content can't be blank, Created by can't be blank/)
      end
    end
  end
  ...
  describe 'PUT /posts/:id' do
    let(:valid_attributes) { { title: 'Shopping', content: 'Lorem Lorem' } }
